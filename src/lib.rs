...
pub async fn convert_squashfs(image_dir: &Path, output_squashfs: &Path, squashfs_binpath: Option<&Path>) -> Result<()> {
...
/// Convert an extracted OCI image directory into a erofs file.
///
/// Layer content is streamed directly into mkfs.erofs's stdin — the merged
/// tar is never fully materialised in memory.
pub async fn convert_erofs(image_dir: &Path, output_erofs: &Path, mkfs_erofs_binpath: Option<&Path>) -> Result<()> {
    let image_dir = image_dir.to_path_buf();
    let output_erofs = output_erofs.to_path_buf();
    let mkfs_erofs_binpath = mkfs_erofs_binpath.map(Path::to_path_buf);

    tokio::task::spawn_blocking(move || {
        let manifest = image::load_manifest(&image_dir)?;
        let layers = image::resolve_layers(&image_dir, &manifest)?;
        erofs::write_erofs(layers, &output_erofs, mkfs_erofs_binpath.as_deref())
    })
    .await?
}
